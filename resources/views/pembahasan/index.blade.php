...
<div class="space-y-6" x-data="{
  ...
  {{-- ── CARD SEARCH & BANNER ────────────────────────── --}}
  <div class="bg-gradient-to-r from-blue-600 to-blue-700 rounded-2xl p-6 text-white shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
    <div class="space-y-2">
  ...
  @if($subjects->isEmpty())
    {{-- ── EMPTY STATE SYSTEM ────────────────────────── --}}
    <div class="bg-white rounded-2xl border border-gray-100 p-12 text-center max-w-xl mx-auto">
      <div class="text-7xl mb-6 select-none animate-pulse">📚</div>
      <h3 class="text-xl font-bold text-gray-800 mb-2">Mata Pelajaran Belum Tersedia</h3>
    ...
    <div class="grid grid-cols-1 gap-4">
      ...
      @foreach($subjects as $subject)
        ...
        <div x-show="matchesSearch('{{ addslashes($subject->name) }}', '{{ addslashes($subject->description) }}')"
          class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden transition-all duration-300 hover:border-gray-200"
          :class="expanded === {{ $subject->id }} ? 'ring-1 ring-blue-500/20 shadow-md' : ''"
          x-transition>

          {{-- Header Accordion --}}
          <div @click="toggle({{ $subject->id }})"
            class="p-5 flex items-center justify-between cursor-pointer select-none hover:bg-gray-50/50 transition-colors">
            <div class="flex items-center gap-4">
              ...
              <div>
                <h3 class="font-bold text-gray-800 text-base leading-tight">{{ $subject->name }}</h3>
            ...
            <div class="flex items-center gap-3">
              <span x-show="expanded !== {{ $subject->id }}" class="text-xs text-blue-600 bg-blue-50 font-semibold px-2.5 py-1 rounded-lg">Buka Bab ▼</span>
              <span x-show="expanded === {{ $subject->id }}" class="text-xs text-gray-500 bg-gray-50 font-semibold px-2.5 py-1 rounded-lg">Tutup ▲</span>
            </div>
          </div>

          {{-- Expandable Content (Topics) --}}
          <div x-show="expanded === {{ $subject->id }}" x-collapse
            class="border-t border-gray-50 bg-gray-50/30 p-6 space-y-4">
            
            @if($subject->description)
              <div class="bg-blue-50/30 border border-blue-100/30 rounded-xl p-4 text-xs text-gray-500 leading-relaxed">
            ...
            @else
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                @foreach($subject->topics as $topic)
                  <div class="bg-white rounded-xl border border-gray-100 p-4 shadow-sm hover:border-blue-100 transition-all flex flex-col justify-between hover:scale-[1.01]">
                    <div>
                      ...
                      <h4 class="font-bold text-gray-800 text-sm mb-1">{{ $topic->name }}</h4>
                    ...
                    <div class="mt-4 pt-3 border-t border-gray-50 flex justify-between items-center gap-2">
      ...
      <div x-show="search.length > 0 && !document.querySelectorAll('.bg-white[x-show*=\'matchesSearch\']').length"
        class="bg-white rounded-2xl border border-gray-100 p-12 text-center max-w-xl mx-auto flex flex-col items-center">
        <div class="text-6xl mb-4 select-none">🔍</div>
        <h4 class="font-bold text-gray-800 mb-1">Mata Pelajaran Tidak Ditemukan</h4>
