...
<div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 max-w-3xl">
    <form action="{{ route('admin.mata-pelajaran.update', $subject->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
                ...
                <select id="class_level" name="class_level" required
                    ...
                    <option value="6" {{ old('class_level', $subject->class_level) == '6' ? 'selected' : '' }}>Kelas 6 SD</option>
                    <option value="9" {{ old('class_level', $subject->class_level) == '9' ? 'selected' : '' }}>Kelas 9 SMP</option>
                    <option value="12" {{ old('class_level', $subject->class_level) == '12' ? 'selected' : '' }}>Kelas 12 SMA/SMK</option>
                </select>
                @error('class_level')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="color" id="color_hex" name="color_hex" value="{{ old('color_hex', $subject->color_hex) }}"
            ...
            <div class="md:col-span-2">
                ...
                <textarea id="description" name="description" rows="3"
                    class="w-full border border-gray-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-blue-500 @error('description') border-red-500 @enderror">{{ old('description', $subject->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
